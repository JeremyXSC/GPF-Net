...
parser.add_argument(
    "--output_dir",
    default = './models_checkpoint',
    type=str,
    help="the output dir for model saving",
)
parser.add_argument(
    "--root_dir",
    default="/home/xiangsuncheng/wxy/resized_frame",
    type=str,
    help="the root for data",
)
...
train_video, val_video, train_list, val_list, train_csv, val_csv, train_index, val_index = (
    data_split(root, file_paths=text_path, fold=fold)
)
...
for epoch in range(max_epoch):
    ...
    with torch.no_grad():
        ...
        for label_item in val_label:
            ...
            for i in range(len(query_list)):
                ...
                ap_tmp, CMC_tmp = evaluate(
                    query_features[i], label_query[i], gallery_features, label_gallery
                )
                CMC_1 += CMC_tmp[0]
                CMC_5 += CMC_tmp[4]
                CMC_10 += CMC_tmp[9]
        ...
        print(
            "Rank@1:%f Rank@5:%f Rank@10:%f mAP:%f" % (CMC_1, CMC_5, CMC_10, ap / len(label_query))
        )
    ...
    logger.info("Rank10 val: %6f" % (CMC_10))
